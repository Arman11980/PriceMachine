...
class PriceMachine:
    ...
    def load_prices(self, file_path='.'):
        ...
        Допустимые названия для столбца с весом (в кг.):
            ...
            фасовка
        """
        for filename in os.listdir(file_path):  # Перебираем файлы в папке
            if 'price' in filename.lower():  # Приводим имя файла к нижнему регистру и проверяем наличие 'price' в имени
                fullpath = os.path.join(file_path, filename)  # Полный путь к файлу
                try:  # Обработка ошибок при открытии и чтении файла
                    with open(fullpath, newline='', encoding='utf-8') as csvfile:
                        ...
                        product_col, price_col, weight_col = self._search_product_price_weight(headers)  # Ищем нужные столбцы
...
pm = PriceMachine()
pm.load_prices()
print(f'Загрузка данных закончена')
